...
#include "scrapers/GamesDBDevelopers.h"
#include "scrapers/GamesDBJSONScraper.h"
#include "scrapers/GamesDBPublishers.h"
...
const std::map<int, std::string> gamesdb_new_genres_map{
    {11, "Sports"},
    {10, "Fighting"},
    {13, "Flight Simulator"},
    {12, "Sandbox"},
...
const std::string GamesDBAPIKey =
...
void thegamesdb_generate_json_scraper_requests(
  ...
  const std::string apiKey = "apikey=" + GamesDBAPIKey;
  ...
  if (usingGameID) {
    ...
    requests.push(std::unique_ptr<ScraperRequest>(
        new TheGamesDBJSONRequest(results, path)));
  } else {
    ...
    if (!platforms.empty()) {
      ...
      for (auto platformIt = platforms.cbegin(); platformIt != platforms.cend();
           platformIt++) {
        ...
        if (mapIt != gamesdb_new_platformid_map.cend()) {
          ...
          LOG(LogWarning)
              << "TheGamesDB scraper warning - no support for platform "
...
void TheGamesDBJSONRequest::process(const std::unique_ptr<HttpReq> &req,
  ...
  if (!boxart.HasMember("base_url") || !boxart.HasMember("data") ||
      !boxart.IsObject()) {
    std::string warn =
        "TheGamesDBJSONRequest - Response include had no usable boxart data.\n";
    LOG(LogWarning) << warn;
    return;
  }
